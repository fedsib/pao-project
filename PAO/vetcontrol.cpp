...
VetControl::VetControl(VetView* parent, PaoDB* pdb, QTableWidget* mainTable, QComboBox* sc, QLineEdit* sled, QDateEdit* sded) : vv(parent), db(pdb),
                       tbl(mainTable), s_combo(sc), s_led(sled), s_ded(sded){}

void VetControl::loadUsers(){
  db->loadUsers();
  putOwnersOnTable();
}
...
void VetControl::putOwnersOnTable() {
  /*
*/
  QMap<QString, AbstractOwner*> us = db->getUserList();
  //for (QMap<QString, Owner*>::iterator i = us.begin(); i != us.end(); ++i){
  foreach (AbstractOwner* abso, db->getUserList()){
    //Owner* abso = i.value();
    Owner *own_ptr = dynamic_cast<Owner*>(abso);
    QString tmpCF = own_ptr->getFiscalCode();
    QString tmpName = own_ptr->getUserInfo()->getName();
    QString tmpSur = own_ptr->getUserInfo()->getSurname();
    QString tmpAddr = own_ptr->getUserInfo()->getContacts().getAddress();
    QString tmpMail = own_ptr->getUserInfo()->getContacts().getMail();
    QString tmpPh = own_ptr->getUserInfo()->getContacts().getPhone();
    /*QString tmpCF = abso->getFiscalCode();
    QString tmpName = abso->getUserInfo()->getName();
    QString tmpSur = abso->getUserInfo()->getSurname();
    QString tmpAddr = abso->getUserInfo()->getContacts().getAddress();
    QString tmpMail = abso->getUserInfo()->getContacts().getMail();
    QString tmpPh = abso->getUserInfo()->getContacts().getPhone();*/
    QStringList nh;
    nh << tmpCF << tmpName << tmpSur << tmpAddr << tmpMail << tmpPh;
    putInTable(nh);               //dangling reference?
  }
...
void VetControl::openEditVisitDialog(){
  ...
  if (vs->hasSelection()){
    ...
    for (int i = 0; i < db->getVisitList()->size() && !trovato; i++){
      ...
      if (vis->getVisitCode() == visitCd){
        //editVisitDialog = new EditVisitDialog(vv,this);
  ...
  //AbstractVisit* tmpVisit = db->obtainVisit(visitCd);
  //if(tmpVisit){
  // db->editVisit(*tmpVisit);
  //}
  //}
  }
  //tbl->clearContents();
